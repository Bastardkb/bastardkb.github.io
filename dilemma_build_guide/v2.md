...
If you ordered a kit with per-key RGB, we will start by installing those. The LEDs are sensitive to heat and need to be installed in a specific way, so make sure to read the instructions below first.

-   Make sure to never stay on the same pad for more than 4 seconds with your soldering iron ! You might risk damaging the pad
-   If a pad doesn't want to be soldered, let it cool down for 30 seconds and then try again
